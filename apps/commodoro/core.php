...
//
			...
			// example $xml_resource => /db/dctl-temp/afd/afd-mondi1552_txt.xml 
...
//                           $xquery .= "\n".' if (matches($e, "<\w+")) then $e else for $x in distinct-values($e) order by $x return <item>{$x}</item> ';
		            ...
		            // qui' figDesc e' inserito in mezzo ad altro testo, lo filtriamo via 
                            $xquery .= ' $node//.[local-name() != "figDesc"]/text() else '; 
                            $xquery .= ' if ($node/parent::*//text() != "") then ';
			    // qui' figDesc e' il solo contenuto del nodo (es: pagina contenento solo l'immagine)				
//                            $xquery .= ' subsequence($node/parent::*//text(), 1)[. >> $node][position() < 5] else ';
                            $xquery .= ' subsequence($node/parent::*//.[local-name() != "figDesc"]/text(), 1)[. >> $node][position() < 5] else ';
                            $xquery .= ' $node//.[local-name() != "figDesc"]/text() ';
                           if ($atPage) {
